...
pub unsafe fn tty_term_read_list(
    ...
    unsafe {
        ...
        let Ok(terminfo) = Database::from_name(cstr_to_str(name)) else {
        ...
        *caps = null_mut();
        ...
        for ent in TTY_TERM_CODES.iter() {
            ...
            let s = match ent.type_ {
                ...
                tty_code_type::String => {
                    let Some(Value::String(s)) = terminfo.raw(cstr_to_str(ent.name.as_ptr()))
                    else {
                        continue;
                    };
                    v = s.to_vec();
                ...
                tty_code_type::Number => {
                    let Some(Value::Number(n)) = terminfo.raw(cstr_to_str(ent.name.as_ptr()))
                    else {
                        continue;
                    };
                ...
                tty_code_type::Flag => {
                    let Some(Value::True) = terminfo.raw(cstr_to_str(ent.name.as_ptr())) else {
                        continue;
                    };
                    c!("1")
                }
...
pub unsafe fn tty_term_string_i(term: *mut tty_term, code: tty_code_code, a: i32) -> Vec<u8> {
    unsafe {
        ...
        let parameters = [Parameter::Number(a)];
        let mut buf = vec![];
        match x.expand(&mut buf, &parameters, &mut (*term).context) {
            Ok(()) => buf,
            Err(err) => {
                log_debug!(
...
pub unsafe fn tty_term_string_ii(
    ...
    unsafe {
        let x = tty_term_string(term, code);
        let parameters = [Parameter::Number(a), Parameter::Number(b)];
        let mut buf = vec![];
        match x.expand(&mut buf, &parameters, &mut (*term).context) {
            Ok(()) => buf,
            Err(err) => {
...
pub unsafe fn tty_term_string_s(term: *mut tty_term, code: tty_code_code, a: *const u8) -> Vec<u8> {
    unsafe {
        ...
        let parameters = [Parameter::String(
            std::slice::from_raw_parts(a, libc::strlen(a)).to_vec(),
        )];
        let mut buf = vec![];
        match x.expand(&mut buf, &parameters, &mut (*term).context) {
            Ok(()) => buf,
...
pub unsafe fn tty_term_string_ss(
    ...
    unsafe {
        let x = tty_term_string(term, code);
        let parameters = [
            Parameter::String(std::slice::from_raw_parts(a, libc::strlen(a)).to_vec()),
            Parameter::String(std::slice::from_raw_parts(b, libc::strlen(b)).to_vec()),
        ];
        let mut buf = vec![];
        match x.expand(&mut buf, &parameters, &mut (*term).context) {
            Ok(()) => buf,
