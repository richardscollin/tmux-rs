...
pub unsafe fn tty_term_override_next(s: *const u8, offset: *mut usize) -> *mut u8 {
    ...
    unsafe {
        ...
        if *s.add(at) == b'\0' as u8 {
            return null_mut();
        }

        while *s.add(at) != b'\0' as u8 {
            if *s.add(at) == b':' as u8 {
                if *s.add(at + 1) == b':' as u8 {
                    value[n] = b':' as u8;
                    n += 1;
                    at += 2;
                } else {
...
pub unsafe fn tty_term_apply(term: *mut tty_term, capabilities: *const u8, quiet: i32) {
    unsafe {
        ...
        while {
            ...
            } else if *s.add(strlen(s) - 1) == b'@' as u8 {
...
pub unsafe fn tty_term_apply_overrides(term: *mut tty_term) {
    ...
    unsafe {
        ...
        unsafe {
            ...
            while *acs != b'\0' as u8 && *acs.add(1) != b'\0' as u8 {
        ...
        'error: {
            ...
            for i in 0..ncaps as usize {
                ...
                for (j, ent) in tty_term_codes.iter().enumerate() {
                    ...
                    if *ent.name.add(namelen) != b'\0' as u8 {
                        continue;
                    }
                    ...
                    match ent.type_ {
                        ...
                        tty_code_type::Flag => {
                            ...
                            (*code).value.flag = (*value == b'1' as u8) as i32;
                        }
                    }
                }
            }
...
pub unsafe fn tty_term_string_i(term: *mut tty_term, code: tty_code_code, a: i32) -> *const u8 {
    unsafe {
        ...
        // s = tiparm(x, a);
        // #else
        let s = tparm(x as *const u8, a, 0, 0, 0, 0, 0, 0, 0, 0);
        // #endif
        if s.is_null() {
            log_debug!(
                "could not expand {}",
...
pub unsafe fn tty_term_string_ii(
    ...
    unsafe {
        ...
        // s = tiparm(x, a, b);
        // #else
        let s = tparm(x as *const u8, a, b, 0, 0, 0, 0, 0, 0, 0);
        // #endif
        if s.is_null() {
            log_debug!(
                "could not expand {}",
...
pub unsafe fn tty_term_string_iii(
    ...
    unsafe {
        ...
        // s = tiparm(x, a, b, c);
        // #else
        let s = tparm(x as *const u8, a, b, c, 0, 0, 0, 0, 0, 0);
        // #endif
        if s.is_null() {
            log_debug!(
                "could not expand {}",
