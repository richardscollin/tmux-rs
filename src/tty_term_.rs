...
use crate::terminfo::find_database;
use crate::terminfo::terminfo_parse;
use crate::*;
...
pub unsafe fn tty_term_read_list(
    ...
    unsafe {
        ...
        let Some(terminfo_path) = find_database(cstr_to_str(name)) else {
        ...
        let Ok(terminfo) = terminfo_parse(&terminfo_buffer) else {
            *cause = format_nul!(
                "can't parse (new) terminfo database for terminal: {}",
                _s(name)
            );
            return -1;
        };
