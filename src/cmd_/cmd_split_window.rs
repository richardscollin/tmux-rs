...
pub static CMD_SPLIT_WINDOW_ENTRY: cmd_entry = cmd_entry {
    ...
    usage: "[-bdefhIPvZ] [-c start-directory] [-e environment] [-F format] [-l size] [-t target-pane] [shell-command [argument ...]]",
...
unsafe fn cmd_split_window_exec(self_: *mut cmd, item: *mut cmdq_item) -> cmd_retval {
    unsafe {
        ...
        if input || (count == 1 && args_string(&*args, 0).unwrap().is_empty()) {
            flags |= SPAWN_EMPTY;
        }
